# Journal des enquêtes de sources

**Ce fichier est une ARCHIVE, pas une référence.** Chaque balayage y est
consigné à sa date, avec la mesure qui l'a tranché : c'est ce qui évite de
refaire une enquête de vingt-trois candidats parce que personne ne se souvient
du verdict.

Ce qu'il faut savoir aujourd'hui tient dans `sources.md`, et l'inventaire des
sources collectées vit dans le code (`packages/collector/src/sources/`). Les
refus réversibles, eux, sont exécutables : `sources/dormant.ts` porte le motif,
la date et la PREUVE qui dirait que la situation a changé.

Un relevé daté vaut pour sa date. Un `robots.txt` change ; un site revient en
ligne. Revérifier avant d'agir sur une fiche ancienne.
